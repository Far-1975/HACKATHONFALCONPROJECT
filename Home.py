...
def diet_planner_ui():
    ...
    st.info("Get a personalized diet plan for disease-free health, adapted to your age, climate, medical history, and health goals.")

    # Form for user details
    with st.form("diet_form"):
        age = st.number_input("Age", min_value=0, max_value=120, step=1)
        gender = st.radio("Gender", ["Male", "Female", "Other"])
        health_goals = st.text_input("Health Goals", placeholder="e.g., Increase immunity, weight loss")
        dietary_preferences = st.selectbox("Dietary Preferences", ["Vegetarian", "Non-Vegetarian", "Vegan", "Pescatarian"])
        st.form_submit_button("Generate Diet Plan")
    
    # Display diet plan results
    st.markdown("**Your diet plan will appear here based on input data.**")

def chatbot_ui():
    st.markdown("### 🤖 Medical Chatbot")
    st.info("Get answers to common medical queries and quick health advice.")
    
    # Chat interface
    with st.form("chat_form"):
        question = st.text_input("Ask the chatbot a question", placeholder="Enter your question...")
        st.form_submit_button("Send")

    st.markdown("**Chatbot response will appear here.**")

def emergency_ui():
    st.markdown("### 🚨 Emergency Services")
    st.warning("Access emergency contacts and health facilities nearby.")

    # Emergency options
    st.button("Call Ambulance")
    st.button("Nearest Hospital")

def eco_market_ui():
    st.markdown("### 🌱 Eco Market")
    st.info("Explore eco-friendly products and medicines. Add items to your cart and check out easily.")

    # Product display with add-to-cart options
    products = ["Vitamin C Supplement", "Reusable Water Bottle", "Organic Green Tea"]
    for product in products:
        st.markdown(f"**{product}**")
        st.button(f"Add {product} to Cart", key=f"cart_{product}")
section = st.sidebar.radio("Go to", ["Home", "Diet Planner", "Chatbot", "Emergency", "Eco Market"])
if section == "Home":
    st.markdown("### 🏠 Home")
    st.write("Welcome to the Doctor's Assistant Dashboard. Use the sidebar to navigate through different features.")
elif section == "Diet Planner":
    diet_planner_ui()
elif section == "Chatbot":
    chatbot_ui()
elif section == "Emergency":
    emergency_ui()
elif section == "Eco Market":
    eco_market_ui()
